...
An excellent fit for the hare counts and a fairly good fit for the lynx counts.

Next we will plot the two time series using the **emude function** `series_plot`. This leverages ggplot functionality but it expects long-format data, so we need to pivot the joined data frame longer and do some additional data wrangling.
...
Create six NODE models, one each for:
...
Then train these models and make one-step-ahead predictions based on the original, relativized, and rescaled data frames.
