...
Instead of going through the rescaling with predict_UDE, do it with the cross-validation function, then
Then use the best one from cross-validation as the train_UDE and predict_UDE.

define the nodes, use cross-validation on all of them, then train and predict on the best one
...
NODE1 <- NODE(data = lynxhare1, time_column_name = "Year")
train_UDE(NODE1)
cv_results <- cross_validation(model = NODE1, k = 4, path = "../tst/cv_test2.csv")
```
