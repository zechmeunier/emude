...
NODE1 = NODE(data = lynxhare1_train, covariates = X, time_column_name = "Year")

train_UDE(NODE1)
NODE1_predictions <- predict_UDE(NODE1, test_data = lynxhare1_test)

rhs = get_right_hand_side(NODE1)
...
phase_plane_2D(observations = obs_long, predictions = preds_long,
                           ...
                           x = Hare, y = Lynx, model = NODE1, covariates = X)

phase_plane_2D(observations = obs_long2, predictions = preds_long2,
                           names = Species, values = Population,
                           x = Hare, y = Lynx, model = UDE1, mag_scale = TRUE)

```

```{r}
get_right_hand_side(UDE1_rel)
```
